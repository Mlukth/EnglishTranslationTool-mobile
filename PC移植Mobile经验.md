...
写 MobileApp.vue 之前，先在 `组件测试/` 目录里为**每个子页面**创建独立的方案对比 Vue 文件。用户逐个看过、确定方案后，再动手写代码。避免写完了用户说不满意要推倒重来。

### 9.2 文件组织

**一个子页面 = 一个文件。** 不要按方案拆分，不要把多个子页面混在一个文件里，不要把组件库和方案对比混在一起。

```
组件测试/
├── ProjectName-mobile-01-功能A.vue    ← 只含功能A的2~4个方案mock
├── ProjectName-mobile-02-功能B.vue    ← 只含功能B的2~4个方案mock
├── ProjectName-mobile-03-功能C.vue    ← ...
└── ...
```
...
```
文件结构（从上到下）：
1. 标题：功能名 + 特点说明（1~2句话讲清楚这个功能有什么特殊需求）
2. 方案横排区：2~4列，每列一个方案
    - 方案名 + badge
    - 240px 手机框 mock（深色主题 #1a1a2e）
    - 优缺点列表（绿色=优点，黄色=缺点）
3. 底部推荐框：推荐哪个方案 + 一句话原因
```

### 9.4 错误做法（不要再犯）

- ❌ 按方案拆分文件（如"方案H详解.vue"里面塞了11个功能）— 用户要看某个功能得跨文件找
- ❌ 一个文件塞所有功能的全部方案（太杂，挑不出）
- ❌ 把可复用组件库和方案对比写在一起（目的不同，组件库是施工物料不是方案展示）
- ❌ 模板里用 JS 注释当占位符（`@click="/* → 全屏 */"` Vue 模板解析报错，用 `@click="() => {}"`）
- ❌ 多个子页面共用同一套 mock 数据（每个功能的数据结构不同，mock 要反映真实差异）

### 9.5 参考模板

参考 `ETT-mobile-弹窗适配方案-DEFG.vue` 的手机框 mock 样式（240px 框、深色主题、方案横排），但**组织方式按本节规范来**（一个功能一个文件）。
